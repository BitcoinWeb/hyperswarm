...
test('bootstrap option', async ({ is }) => {
  ...
  const swarm = bitswarm({ bootstrap })
  promisifyMethod(swarm, 'listen')
  await swarm.listen()
...
test('connect two peers directly', async ({ is }) => {
  const swarm1 = bitswarm({ bootstrap: [] })
  const swarm2 = bitswarm({ bootstrap: [] })
  swarm1.listen()
...
test('connect two peers using join (announcing peer and lookup peer)', async ({ is }) => {
  ...
  const peer1 = bitswarm({ bootstrap })
  const peer2 = bitswarm({ bootstrap })
...
test('can multiplex 100 topics over the same connection', async ({ same }) => {
  const { bootstrap, closeDht } = await dhtBootstrap()
  const swarm1 = bitswarm({ bootstrap, maxPeers: 20, queue: { multiplex: true } })
  const swarm2 = bitswarm({ bootstrap, maxPeers: 20, queue: { multiplex: true } })
